...
void read_cb(void* target, uint64_t value, faster_status status) {
  assert(status == Ok);
}
...
uint64_t rmw_cb(const uint8_t* current, uint64_t length, uint8_t* modification, uint64_t modification_length, uint8_t* dst) {
  ...
  uint8_t val[1] = {static_cast<uint8_t>(current[0] + modification[0])};
  memcpy(dst, val, 1);
  return 1;
}
...
void thread_setup_store(faster_t* store, size_t thread_idx) {
  ...
  for(uint64_t chunk_idx = idx_.fetch_add(kChunkSize); chunk_idx < kInitCount;
    ...
    for(uint64_t idx = chunk_idx; idx < chunk_idx + kChunkSize; ++idx) {
      ...
      faster_upsert(store, init_keys_.get()[idx], value);
    }
  }
...
void thread_run_benchmark(faster_t* store, size_t thread_idx) {
  ...
  while(!done_) {
    ...
    for(uint64_t idx = chunk_idx; idx < chunk_idx + kChunkSize; ++idx) {
      ...
      case Op::Insert:
      case Op::Upsert: {
        faster_upsert(store, txn_keys_.get()[idx], upsert_value);
        ++writes_done;
        break;
      }
      case Op::Scan:
        ...
        break;
      case Op::Read: {
        faster_read(store, txn_keys_.get()[idx], read_cb, NULL);
        ++reads_done;
        break;
      }
      case Op::ReadModifyWrite:
        uint8_t result = faster_rmw(store, txn_keys_.get()[idx], 5);
        if(result == 0) {
          ++writes_done;
        }
        break;
